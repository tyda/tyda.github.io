(function($) {
    ...
    $(document).ready(function() {		
		...
		$(".thumbnail-position").delegate('.opento', 'click', function(event) {
			event.stopImmediatePropagation();
			event.preventDefault();
			var $this = $(this);
			$("html,body").animate({scrollTop: $(".window-position").offset().top -270 }, 500);
			$(".window-position").stop().slideUp('slow', function(){
				$.ajax({
					type: "get",
					url: $this.attr('href'),
					success: function(data){
						var $htmlContent = $("<div/>").html(data);
						$(".window-position").html($htmlContent.find('.portfolio-item').html());
						$(".window-position").imagesLoaded(function(){
							$('.window-position').fitVids();
							if($('.window-position .slider_flexslider').length>0){
								setTimeout( function(){
									setFlexSlider('.window-position');
								}, 500 );
							}
							$(".window-position").stop().slideDown('slow');
						});
						
						$(".window-position .detail-portfolio").delegate('i.close', 'click', function() {
							$(".window-position").slideUp('slow');
						});
					}
				});
			});
